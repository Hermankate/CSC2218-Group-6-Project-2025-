...
# # #                         )
# # #                     ),
# # #                     ft.Row([  # Wrap share and delete in a Row
# # #                         share_btn,
# # #                         delete_btn
# # #                     ], spacing=5)
# # #                 ], alignment="spaceBetween"),
...
#             if current_category == "Tagged":
#                 if api.token:
#                     current_user_id = page.client_storage.get("current_user", {}).get('id')
#                     filtered_notes = [
#                         n for n in filtered_notes 
#                         if current_user_id in [u.get('id') for u in n.get('tagged_users', [])]
#                     ]
...
#                         if not any(n['local_id'] == server_note.get('local_id') for n in local_notes):
#                             merged_notes.append(server_note)
                    
#                     for local_note in local_notes:
#                         if local_note.get('local_id') not in local_ids:
#                             merged_notes.append(local_note)
